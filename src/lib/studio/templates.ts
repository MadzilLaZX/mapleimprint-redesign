// Maple Template system (Designs/Templates upgrade). A GRAPHIC is one asset; a TEMPLATE is a whole
// editable composition (text + graphics + layout) built from the same normalized DesignObject
// model as a DesignProject. Applying a template deep-copies its objects (fresh ids) into the
// customer's active side — Studio never keeps a live reference back to the template, so editing a
// customer's design can never mutate (or be mutated by) the shared library.
//
// Every object's normalizedX/Y/Width/Height are already fractions (0-1) of whichever print-area
// box is active when the template is applied (see StudioClient.applyTemplate + CanvasStage's
// boxFor()) — that's what makes a template automatically "fit" a left-chest box as sensibly as a
// full front box, with no per-print-area coordinate math needed here (Section 8 of the brief).
//
// The templates below are Maple-owned internal demos only (shapes, typography, and the hand-built
// marks in assetProviders.ts) — proof of the UX, explicitly not a purchased/scraped library. No
// third-party or trademarked content is used anywhere here.
...
export interface DesignTemplate {
  ...
  /** Locations this composition was designed to look good in. Advisory metadata (the normalized
   *  coordinate system already makes every template render sensibly in any print area — see the
   *  file header) used for "recommended for this placement" hints, not a hard gate. */
...
export const MAPLE_TEMPLATES: DesignTemplate[] = [...SPECS.map(build), ...CREATIVE_SPECS.map(cbuild), ...PRO_SPECS.map(pbuild)];
